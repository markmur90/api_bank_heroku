...
class ProcessingOrderMetadataEventsInner(BaseModel):
    """
    ProcessingOrderMetadataEventsInner
    """ # noqa: E501
    timestamp: datetime
    name: StrictStr
    __properties: ClassVar[List[str]] = ["timestamp", "name"]

    @field_validator('name')
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['INBOX_ORIGINATOR', 'SCAN_DATE']):
            raise ValueError("must be one of enum values ('INBOX_ORIGINATOR', 'SCAN_DATE')")
        return value
