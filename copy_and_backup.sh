...
SOURCE="/home/markmur88/Documentos/GitHub/api_bank_h/"
...
BACKUP_ZIP="${BACKUP_DIR}$(date +%Y-%m-%d_%H-%M-%S)_backup_api_bank_h.zip"
...
if [[ "$CONFIRM" != "s" ]]; then
    ...
    rsync -av --exclude=".gitattributes" --exclude=".gitignore" --exclude="auto_commit_sync.sh" --exclude="copy_and_backup.sh" --exclude=".git/" "$SOURCE" "$DEST"
    echo " "
...
if [[ "$CONFIRM" == "s" ]]; then
    ...
    # Realizar la copia de seguridad en formato ZIP sin excluir archivos
